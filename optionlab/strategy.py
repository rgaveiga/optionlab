...
from optionlab.black_scholes import (getBSinfo,
                                     getimpliedvol)
from optionlab.support import (getPLprofile,
                               getPLprofilestock,
                               getPLprofileBS,
                               getprofitrange,
                               getnonbusinessdays,
...
class Strategy:      
            ...
            if strategy[i]["type"] in ("call","put"):
                ...
                if use_dates:
                    ...
                    if days2maturitytmp>=self.__days2target:
                        ...
                        raise ValueError("Days remaining to maturity must be greater than or equal to the number of days remaining to the target date!")                    
            ...
            for strategy in d["Strategy"]:
                ...
                if strategy["type"] in ("call","put"):
                    ...
                    if "expiration" in strategy.keys():
                        if self.__use_dates:
                            expirationtmp=datetime.strptime(strategy["expiration"],
                                                            "%Y-%m-%d").date()
                        else:
    ...
    def run(self):
        ...
        if self.__compute_expectation and self.__s_mc.shape[0]==0:             
            self.__s_mc=createpricesamples(self.__stockprice,
                                           self.__volatility,
                                           time2target,
                                           self.__r,
                                           self.__distribution,
        ...
        for i in range(len(self.__type)):                     
            if self.__type[i] in ("call","put"):
                ...
                if self.__prevpos[i]<0.0: # Previous position is closed
                    ...
                    if self.__usebs[i]:
                        self.profit[i],self.cost[i]=getPLprofileBS(self.__type[i],
                                                                   self.__action[i],
                                                                   self.__strike[i],
                                                                   opval,
                                                                   self.__r,
                                                                   (self.__days2maturity[i]-self.__days2target)/self.__daysinyear,
                                                                   self.__volatility,
                                                                   self.__n[i],
                                                                   self.__s,
                                                                   self.__y,
                ...
                if self.__prevpos[i]<0.0: # Previous position is closed
                    ...
                    self.profit[i],self.cost[i]=getPLprofilestock(stockpos,
                                                                  self.__action[i],
                                                                  self.__n[i],
                                                                  self.__s,
        ...
        if self.__profittarg is not None:
            self.__profittargrange=getprofitrange(self.__s,self.strategyprofit,
                                                  self.__profittarg)
        ...
        if self.__losslimit is not None:
            self.__losslimitranges=getprofitrange(self.__s,self.strategyprofit,
        ...
        if (self.__compute_expectation or self.__distribution=="array") and\
            ...
            output["ProbabilityOfProfitFromMC"]=(self.strategyprofit_mc>=0.01).sum()\
                /self.strategyprofit_mc.shape[0]
    ...
    def plotPL(self):
        ...
        if self.__losslimit is not None and self.__profittarg is not None:
            plt.plot(self.__s,zeroline,'m--',
                     self.__s,lossline,'r--',
                     self.__s,targetline,'b--',
                     self.__s,self.strategyprofit,'k-',
                     strikecallbuy,zerocallbuy,'b>',
                     strikeputbuy,zeroputbuy,'b<',
                     strikecallsell,zerocallsell,'r>',
                     strikeputsell,zeroputsell,'r<',
                     markersize=10)        
