...
def get_bs_info(
    s: float, x: float, r: float, vol: float, time_to_maturity: float, y: float = 0.0
) -> BlackScholesInfo:
    """
    get_bs_info(s, x, r, vol, time_to_maturity, y) -> provides informaton about call and
    ...
    """
    d1, d2 = get_d1_d2(s, x, r, vol, time_to_maturity, y)
    call_price = get_option_price("call", s, x, r, time_to_maturity, d1, d2, y)
    put_price = get_option_price("put", s, x, r, time_to_maturity, d1, d2, y)
    call_delta = get_delta("call", d1, time_to_maturity, y)
    put_delta = get_delta("put", d1, time_to_maturity, y)
    call_theta = get_theta("call", s, x, r, vol, time_to_maturity, d1, d2, y)
    put_theta = get_theta("put", s, x, r, vol, time_to_maturity, d1, d2, y)
    gamma = get_gamma(s, vol, time_to_maturity, d1, y)
    vega = get_vega(s, time_to_maturity, d1, y)
    call_itm_prob = _get_itm_probability("call", d2, time_to_maturity, y)
    put_itm_prob = _get_itm_probability("put", d2, time_to_maturity, y)
...
def get_option_price(
    ...
    """
    get_option_price(op_type, s0, x, r, time_to_maturity, d1, d2, y) -> returns the price of
...
def get_delta(
    op_type: OptionType, d1: float, time_to_maturity: float = 0.0, y: float = 0.0
) -> float:
    """
    get_delta(op_type, d1, time_to_maturity, y) -> computes the Greek Delta for an option
    ...
    """
    if y > 0.0 and time_to_maturity > 0.0:
        yfac = exp(-y * time_to_maturity)
    else:
        yfac = 1.0
...
def get_gamma(
    s0: float, vol: float, time_to_maturity: float, d1: float, y: float = 0.0
) -> float:
    """
    get_gamma(s0, vol, time_to_maturity, d1, y) -> computes the Greek Gamma for an option
    ...
    return yfac * cdf_d1_prime / (s0 * vol * sqrt(time_to_maturity))
...
def get_theta(
    ...
    """
    get_theta(op_type, s0, x, r, vol, time_to_maturity, d1, d2, y) -> computes the Greek Theta
    ...
    if op_type == "call":
        return -(
            s * vol * cdf_d1_prime / (2.0 * sqrt(time_to_maturity))
            + r * x * exp(-r * time_to_maturity) * stats.norm.cdf(d2)
...
def get_vega(s0: float, time_to_maturity: float, d1: float, y: float = 0.0) -> float:
    """
    get_vega(s0, time_to_maturity, d1) -> computes the Greek Vega for an option taking
    ...
    return s * cdf_d1_prime * sqrt(time_to_maturity) / 100
...
def get_d1_d2(
    ...
    """
    get_d1_d2(s0, x, r, vol, time_to_maturity, y) -> returns 'd1' and 'd2' taking the
    ...
    """
    d1 = (log(s0 / x) + (r - y + vol * vol / 2.0) * time_to_maturity) / (
...
def _get_implied_vol(
    ...
    """
    _get_implied_vol(op_type, oprice, s0, x, r, time_to_maturity, y) -> estimates the implied
    ...
    d1, d2 = get_d1_d2(s0, x, r, vol, time_to_maturity, y)
    dopt = abs(
        get_option_price(op_type, s0, x, r, time_to_maturity, d1, d2, y) - oprice
    )
...
def _get_itm_probability(
    op_type: OptionType, d2: float, time_to_maturity: float = 0.0, y: float = 0.0
) -> float:
    """
    getitmprob(op_type,d2,time_to_maturity,y) -> returns the estimated probability
